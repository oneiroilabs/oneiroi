...
struct Uniforms {
    ...
    tube_radius: f32,
    radial_segments: u32, // Anzahl der Ecken des Kreises (z.B. 16 oder 32)
    _pad0: u32,
    _pad1: u32,
}
@group(0) @binding(1)
...
fn vs_main(
    ...
    // 1. Bestimmen, welcher Punkt im Ring und ob wir im vorderen oder hinteren Ring sind
    // Wir bauen ein Triangle-Strip-Muster pro Instanz auf
    let total_subdivisions = config.radial_segments;
    let ring_quad_vertex = vert_idx % 6u;
    let segment_pixel_idx = vert_idx / 6u; // Welches "Tortenstück" des Kreises

    var local_vertex_id = segment_pixel_idx;
    var use_next_frame = 0u;

    // Custom Index-Mapping für 2 Triangles (Ein Quad) ohne Index-Buffer:
    // 0 -> (id, current), 1 -> (id, next), 2 -> (id+1, current)
    // 3 -> (id+1, current), 4 -> (id, next), 5 -> (id+1, next)
    ...
    // 2. Den korrekten RMF-Frame aus dem Storage-Buffer laden
    let target_frame_idx = inst_idx + use_next_frame;
    let frame = evaluated_frames[target_frame_idx];

    // 3. Kreiskoordinaten im lokalen 2D-Raum berechnen
    let angle = (f32(local_vertex_id) / f32(total_subdivisions)) * 2.0 * PI;
    let cos_a = cos(angle);
    let sin_a = sin(angle);

    // 4. Extrusion: Den 2D-Kreis mittels Normalen und Binormalen in den 3D-Raum transformieren
    ...
    // 5. Outputs für den Fragment-Shader füllen
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    // Einfaches Richtungs-Lighting (Diffuse Shading) basierend auf der Röhren-Normale
